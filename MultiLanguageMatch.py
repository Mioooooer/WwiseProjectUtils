...
def BatchProcessSelected(waapiClient, targetTypeList, property, value, workerFunction, recursion = True):
    idList = []
    for targetType in targetTypeList:
        for id in getSingleSelectedInfoList(waapiClient, "id"):
            idList += recursionFindProperty(waapiClient, id, "type", targetType, recursion)
    workerFunction(waapiClient, idList, property, value)
...
def importVoice(waapiClient, idList, property, value):
    for child in idList:
        tarPath = getSingleInfoBy(waapiClient, child, "path")
        for filePath in property:
            ...
            Result = waapiClient.call("ak.wwise.core.audio.import", args)
            waapiClient.call("ak.wwise.core.log.addItem", {"channel": "general", "severity": "Message", "message": "[" + lanStr + "] " + nameStr + " updated"})
...
try:
    ...
    with WaapiClient() as client:
        ...
        #targetTypeList = ["WorkUnit", "Folder", "ActorMixer"]
        targetTypeList = args.targetTypeList
        logChannel = ""
        #if args.Channel == "soundbankGenerate":
        #    logChannel = "soundbankGenerate"
        recursion = False
        inputPath = input("drag/type in your audio file/folder")
        targetPathList = []
        if os.path.isdir(inputPath):
            # 使用方法
            directory = inputPath
            targetPathList = list_all_files(directory)
        else:
            targetPathList.append(inputPath)
        property = targetPathList
        BatchProcessSelected(client, targetTypeList, property, logChannel, importVoice, recursion)
